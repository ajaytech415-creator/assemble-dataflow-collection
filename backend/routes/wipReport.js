...
export const generateWipExcelBuffer = async (startDate, endDate) => {
  ...
  const calcStats = (name, category, inMos, outMos, scrap, returns, reworks) => {
    ...
    inMos.forEach(e => {
      if (e[nameField] === name) IN += e[qtyField] !== undefined ? (e[qtyField] || 0) : (e.qty || 0);
    });

    outMos.forEach(e => {
      if (e[nameField] === name) OUT += e[compField] || 0;
    });
  ...
  wsData.push([
    'MATERIAL / PART DESCRIPTION', 'IN (Planned)', 'RC (Received)', 'RJ (Rejected)', 'RT (Returned)', 'OUT (Completed)', 'WIP — All Time', '',
    hasPeriod ? 'IN (Period)'  : '', hasPeriod ? 'RC (Period)'  : '', hasPeriod ? 'RJ (Period)'  : '', hasPeriod ? 'RT (Period)'  : '', hasPeriod ? 'OUT (Period)' : '', hasPeriod ? 'WIP — Period' : '',
  ]);
  ...
  categories.forEach(cat => {
    const names = (comps[cat.key] || []).map(m => (typeof m === 'string' ? m : m.name)).filter(Boolean);
    wsData.push([`— ${cat.label.toUpperCase()} —`]);
    names.forEach(name => {
      const total  = calcStats(name, cat.key, allMOs,      allMOs,       allScrap,   allReturns, allReworks);
      const period = hasPeriod ? calcStats(name, cat.key, periodInMOs, periodOutMOs, periodScrap, periodReturns, periodReworks) : total;

      grandIN  += total.IN; grandRC  += total.RC; grandRJ  += total.RJ; grandRT  += total.RT; grandOUT += total.OUT;
      grandPIN  += period.IN; grandPRC  += period.RC; grandPRJ  += period.RJ; grandPRT  += period.RT; grandPOUT += period.OUT;

      wsData.push([
        name, total.IN, total.RC, total.RJ, total.RT, total.OUT, total.WIP, '',
        hasPeriod ? period.IN  : '', hasPeriod ? period.RC  : '', hasPeriod ? period.RJ  : '', hasPeriod ? period.RT  : '', hasPeriod ? period.OUT : '', hasPeriod ? period.WIP : '',
      ]);
    });
  });
  ...
  wsData.push([]);
  wsData.push([
    '★ GRAND TOTAL (All Components)', grandIN, grandRC, grandRJ, grandRT, grandOUT, grandWIP, '',
    hasPeriod ? grandPIN  : '', hasPeriod ? grandPRC  : '', hasPeriod ? grandPRJ  : '', hasPeriod ? grandPRT  : '', hasPeriod ? grandPOUT : '', hasPeriod ? grandPWIP : '',
  ]);

  const ws = XLSX.utils.aoa_to_sheet(wsData);
  ws['!cols'] = [{ wch: 40 },{ wch: 14 },{ wch: 14 },{ wch: 14 },{ wch: 14 },{ wch: 14 },{ wch: 14 },{ wch: 4  },{ wch: 12 },{ wch: 12 },{ wch: 12 },{ wch: 12 },{ wch: 12 },{ wch: 12 }];
  ...
  for (let R = range.s.r; R <= range.e.r; ++R) {
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      cell.s = { font: { name: 'Calibri', sz: 11 }, alignment: { vertical: 'center' }, border: { top: { style: 'thin', color: { rgb: 'E5E7EB' } }, bottom: { style: 'thin', color: { rgb: 'E5E7EB' } }, left: { style: 'thin', color: { rgb: 'E5E7EB' } }, right: { style: 'thin', color: { rgb: 'E5E7EB' } } } };
      const cellVal = String(cell.v || '');

      if (R === 0) { cell.s.font = { name: 'Calibri', sz: 18, bold: true, color: { rgb: '1E3A8A' } }; cell.s.border = {}; }
      else if (R === 1) { cell.s.font = { name: 'Calibri', sz: 12, color: { rgb: '6B7280' } }; cell.s.border = {}; }
      else if (R === 2) { cell.s.font = { name: 'Calibri', sz: 11, italic: true, color: { rgb: '059669' } }; cell.s.border = {}; }
      else if (R === 4) { if (cell.v && String(cell.v).startsWith('◄')) { cell.s.font = { name: 'Calibri', sz: 11, bold: true, color: { rgb: 'FFFFFF' } }; cell.s.fill = { fgColor: { rgb: C < 8 ? '1D4ED8' : '059669' } }; cell.s.alignment = { vertical: 'center', horizontal: 'center' }; cell.s.border = {}; } }
      else if (R === 5) { cell.s.font = { name: 'Calibri', sz: 11, bold: true, color: { rgb: 'FFFFFF' } }; cell.s.fill = { fgColor: { rgb: C < 8 ? '1D4ED8' : '065F46' } }; cell.s.alignment = { vertical: 'center', horizontal: 'center' }; }
      else if (cellVal.startsWith('— ')) { cell.s.font = { name: 'Calibri', sz: 11, bold: true, color: { rgb: '991B1B' } }; cell.s.fill = { fgColor: { rgb: 'FEE2E2' } }; cell.s.alignment = { vertical: 'center', horizontal: 'left' }; }
      else if (cellVal.startsWith('★')) { cell.s.font = { name: 'Calibri', sz: 12, bold: true, color: { rgb: 'FFFFFF' } }; cell.s.fill = { fgColor: { rgb: '064E3B' } }; cell.s.alignment = { vertical: 'center', horizontal: C === 0 ? 'left' : 'center' }; }
      else if (C > 0 && R > 5) { cell.s.alignment = { vertical: 'center', horizontal: 'center' }; if ((C === 6 || C === 13) && typeof cell.v === 'number') { cell.s.font = { name: 'Calibri', sz: 11, bold: true, color: { rgb: cell.v < 0 ? 'DC2626' : (cell.v === 0 ? '6B7280' : '059669') } }; } }
    }
  }
  ...
  const filename = startDate && endDate ? `WIP_Report_${safeStart}_to_${safeEnd}.xlsx` : `WIP_Report_AllTime.xlsx`;
...
export const downloadWipExcel = async (req, res) => {
  try {
    ...
    const { buf, filename } = await generateWipExcelBuffer(startDate, endDate);
